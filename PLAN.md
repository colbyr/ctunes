...
| S2 Siri entities and search | done | the `.audio` schema entities, `AudioSearchQuery` and `PlayAudioIntent`; deployment target 27; the catalog on `AppRuntime`; simulator runs through `CTUNES_DEV_INTENT` resolved and played an artist, album and song, Play Next, "play music", and the same offline; Siri on the phone is a hand check |
