...
iCloud enabled on the App ID; a build signed without it silently stores
locally and nothing syncs. **A veto is a `Veto`: an artist, album or track
target plus the title it was saved with**, since a track can't be named
from any list the app holds; `vetoes` replaces the old `vetoedArtistKeys`,
which still decodes (titles empty, named from the library) and is still
written for older builds. `roster.hidden` is a `VetoSet` of the active
listeners' keys per kind, with one rule everywhere: an item is hidden by
its own veto or any wider one (`hides(album)`, `hides(track)`), and
playing a collection skips only what is hidden *inside* it
(`hides(track, within: .album)` counts the track's veto alone, `.artist`
the track's and its album's), because the collection itself was opened on
purpose. Vetoes never absorb each other: un-vetoing an artist leaves
their album vetoes in place. The artist page's avatars toggle the artist,
the album page's the album, and `HiddenRightNowLabel` explains when a
wider veto is what hides the page. Every artist, album and track menu has
the Listeners submenu (`ListenersMenu`, fed a `VetoScope`), which disables
a listener's toggle when a wider veto already covers the item. The
listener's page lists every veto in one list, artists first, a caption
telling the kinds apart; artists are added there from the library, albums
and tracks only from their menus and pages.
